                    ...
                    <div className="p-4 rounded-2xl bg-zinc-950 border border-zinc-800 space-y-3">
                      ...
                      <div className="p-3.5 bg-zinc-900/90 rounded-xl border border-blue-500/30 text-xs space-y-2">
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 p-2.5 bg-black/60 rounded-lg border border-zinc-800 font-mono">
                          <div>
                            <p className="text-[10px] text-zinc-400 uppercase">IP Whitelist Rule for Atlas Console:</p>
                            <p className="text-sm font-extrabold text-emerald-400 select-all tracking-wider">
                              {mongoStatus?.whitelistCidr || mongoStatus?.allowAnyCidr || '0.0.0.0/0'}
                            </p>
                          </div>
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              const cidr = mongoStatus?.whitelistCidr || mongoStatus?.allowAnyCidr || '0.0.0.0/0';
                              navigator.clipboard.writeText(cidr);
                              addToast(`Copied ${cidr} to clipboard!`, 'success');
                            }}
                        ...
                        <p className="text-[11px] text-zinc-300 leading-snug font-sans">
                          In your MongoDB Atlas console, navigate to <strong className="text-white">Security &gt; Network Access &gt; + Add IP Address</strong> and add <code className="bg-zinc-800 text-emerald-300 px-1 py-0.5 rounded font-mono font-bold">{mongoStatus?.allowAnyCidr || '0.0.0.0/0'}</code> (Allow Access from Anywhere) to permit dynamic Cloud Run container traffic.
                        </p>
                      </div>
